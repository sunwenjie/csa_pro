...
import com.asgab.entity.UserJson;
...
//  @RequiresRoles(value = {"admin"})
  public String list(@RequestParam(value = "pageNumber", defaultValue = "1") int pageNumber,
    ...
    // 添加状态
    model.addAttribute("statuses", getStatuses(request));
    return "account/usersList";
  }

  private Map<String, String> getStatuses(ServletRequest request) {
  ...
  public String all() {
    ...
    for (int i = 0; i < users.size(); i++) {
      if (StringUtils.isNotBlank(users.get(i).getName())) {
        UserJson tmpUserJson = new UserJson();
        tmpUserJson.setId(users.get(i).getId());
        tmpUserJson.setName(users.get(i).getName());
        userJsons.add(tmpUserJson);
      }
    }
    return JsonMapper.nonDefaultMapper().toJson(userJsons);
  }
